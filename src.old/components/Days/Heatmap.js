...
function Heatmap({ days, height, maxWidth, mode }) {
  const getKcalSum = (day) => {
    return day
      ? Object.values(day.activities).reduce((a, b) => a + b.calories, 0)
      : 0;
  };

  const getTimeSum = (day) => {
    return day
      ? Object.values(day.activities).reduce((a, b) => a + b.duration, 0)
      : 0;
  };
  ...
  const getSliderSteps = () => {
    ...
    const steps = days.map((day, i) => {
      ...
      const color =
        mode === 'calories' ? getColorFromCalories(sum) : getColorFromTime(sum);
  ...
  return (
    <HeatSlider steps={getSliderSteps()} height={height} maxWidth={maxWidth} />
  );
}
