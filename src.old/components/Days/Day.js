...
function Day({
  ...
  const walks =
    (activities.filter((a) => a.type === 'Walk').length / aCount) * 100;
  const gyms =
    (activities.filter((a) => a.type === 'Gym').length / aCount) * 100 + walks;
  const cycles =
    (activities.filter((a) => a.type === 'Cycle').length / aCount) * 100 + gyms;
  const runs =
    (activities.filter((a) => a.type === 'Run').length / aCount) * 100 + cycles;
  const others =
    (activities.filter((a) => !['Walk', 'Run', 'Cycle', 'Gym'].includes(a.type))
      .length /
      aCount) *
      100 +
    runs;
