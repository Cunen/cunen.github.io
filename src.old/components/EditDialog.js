...
function EditDialog({
  ...
  const saveAndClose = async () => {
    ...
    const activityIndex = clonedActivities.findIndex(
      (a) => a.id === activity.id
    );
    if (activityIndex !== -1) {
      clonedActivities[activityIndex] = {
        ...activityDoc.data(),
        id: activity.id,
      };
    }
  ...
  return (
    <Dialog open={!!activity} onClose={saveAndClose}>
      <DialogTitle
        style={{
          display: 'flex',
          flexDirection: 'row',
        ...
        >
          ...
          <InputWrapper>
            ...
            {type !== 'Gym' && (
              <>
                {simple && (
                  <Typography>Distance ({distance / 1000} km)</Typography>
                )}
                {simple ? (
                  <Slider
                    value={distance / 1000}
                    ...
                    onChange={handleSimpleDistanceChange}
                  />
                ) : (
                  <TextField
                    fullWidth
            ...
            {simple && (
              <Typography>Calories ({calories.toFixed(0)} kcal)</Typography>
            )}
            {simple ? (
              <Slider
                value={calories}
