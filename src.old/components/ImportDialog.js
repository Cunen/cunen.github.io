...
function ImportDialog({ db, dbRef, userCollection, open, close }) {
  const [clientID, setClientID] = React.useState(
    window.localStorage.getItem(storedcid)
  );
  const [clientSecret, setClientSecret] = React.useState(
    window.localStorage.getItem(storedsecret)
  );
  ...
  const runStravaImport = async () => {
    ...
    while (rows === 200) {
      const imported = await axios.get(
        `https://www.strava.com/api/v3/athlete/activities?access_token=${token}&per_page=200&page=${page}${beforeEpoc}${afterEpoc}`
      );
      if (imported.data) {
  ...
  return (
    <Dialog open={open} onClose={handleModalClose}>
      ...
      <DialogContent>
        <Typography>
          Don't fuck with this if you don't know what you are doing
        </Typography>
        <Wrapper>
          {!code && (
            <>
